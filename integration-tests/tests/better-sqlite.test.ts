...
test.beforeEach((t) => {
  ...
  ctx.db.run(sql`
		create table ${coursesTable} (
			id integer primary key,
			name text not null,
			category_id integer references ${courseCategoriesTable}(${name(
...
test.serial('table configs: unique third param', (t) => {
  ...
  t.assert(
    tableConfig.uniqueConstraints[0]?.name ===
      uniqueKeyName(
        cities1Table,
        tableConfig.uniqueConstraints[0]?.columns?.map(
          (column) => column.name
        ) ?? []
      )
  );
...
test.serial('table configs: unique in column', (t) => {
  ...
  t.assert(
    columnField?.uniqueName === uniqueKeyName(cities1Table, [columnField!.name])
  );
});
...
test.serial('insert via db.run + select via db.all', (t) => {
  ...
  const result = db.all<{ id: number; name: string }>(
...
test.serial('Insert all defaults in 1 row', (t) => {
  const { db } = t.context;
...
test.serial('Insert all defaults in multiple rows', (t) => {
  const { db } = t.context;
...
test.serial('view', (t) => {
  ...
  const newYorkers1 = sqliteView('new_yorkers1').as((qb) =>
  ...
  db.run(
    sql`create view ${newYorkers2} as ${getViewConfig(newYorkers2).query}`
  );
...
test.serial(
  'insert with onConflict do nothing using composite pk as target',
  (t) => {
    const { db } = t.context;
...
test.serial(
  'async api - insert + select w/ prepare + async execute',
  async (t) => {
    const { db } = t.context;
...
test.serial.only('text w/ json mode', (t) => {
  ...
  db.run(
    sql`create table ${test} (data text not null, data_typed text not null)`
  );

  db.insert(test)
    .values({ data: { foo: 'bar' }, dataTyped: { a: 1 } })
    .run();
